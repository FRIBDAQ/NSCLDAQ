...
CMDPP32SCP::onAttach(XXUSB::CConfigurableObject& configuration)
{
  ...
  // not supportable m_pConfiguration -> addBooleanParameter("-printregisters",  false);
...
CMDPP32SCP::Initialize(CVMUSB& controller)
{
  ...
  for (uint16_t channelPair = 0; channelPair < 8; channelPair++) {
    ...
    list.vmeWrite16(base + TFIntDiff,           initamod, (uint16_t)tfintdiff.at(channelPair));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + PZ0,                 initamod, (uint16_t)pz.at(channelPair*4));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + PZ1,                 initamod, (uint16_t)pz.at(channelPair*4 + 1));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + PZ2,                 initamod, (uint16_t)pz.at(channelPair*4 + 2));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + PZ3,                 initamod, (uint16_t)pz.at(channelPair*4 + 3));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + Gain,                initamod, (uint16_t)gain.at(channelPair));
    list.delay(MDPPCHCONFIGDELAY);
    ...
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + ShapingTime,         initamod, (uint16_t)shapingtime.at(channelPair));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + BLR,                 initamod, (uint16_t)blr.at(channelPair));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + SignalRiseTime,      initamod, (uint16_t)signalrisetime.at(channelPair));
    list.delay(MDPPCHCONFIGDELAY);
