...
*/
void CV977::onAttach(XXUSB::CConfigurableObject& configuration)
{
...
CV977::Initialize(CVMUSB& controller)
{
  ...
  // MVLC - can't use lists at init time.

  CVMUSB&   list(controller);
  list.vmeWrite16(base + InputMask,  amod, inputMask);
  list.vmeWrite16(base + OutputMask, amod, outputMask);
  list.vmeWrite16(base + Ipl,        amod, ipl);
  list.vmeWrite16(base + Vector,     amod, vector);
  ...
  list.vmeWrite16(base + Control, amod, csrValue);
