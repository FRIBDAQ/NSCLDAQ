...
CMDPP16QDC::onAttach(XXUSB::CConfigurableObject& configuration)
{
  ...
  m_pConfiguration -> addIntegerParameter("-triggeroutput", 0, 0x400, 0x400);
	...
	m_pConfiguration -> addIntegerParameter("-setmonitorch",  0,    15,     0);
...
CMDPP16QDC::Initialize(CVMUSB& controller)
{
  ...
  CVMUSB& list(controller);	// Initialization instructions will be added to this.
  ...
  for (uint16_t iIncr = 0; iIncr < 7; iIncr++) {
    list.vmeWrite16(base + TrigToIRQ1L + 4*iIncr, initamod, (uint16_t)trigtoirq.at(iIncr));
  }
  ...
  for (uint16_t channelPair = 0; channelPair < 8; channelPair++) {
    ...
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + Threshold0,          initamod, (uint16_t)threshold.at(channelPair*2));
    list.delay(MDPPCHCONFIGDELAY);
    list.vmeWrite16(base + Threshold1,          initamod, (uint16_t)threshold.at(channelPair*2 + 1));
    list.delay(MDPPCHCONFIGDELAY);
