...
`configure`, nothing is written to the prefix before the install step. When a
package is used from the source tree, editing its sources makes the next
build rebuild and restage it.
...
inside an `assert()` in `daq/eventbuilder/transmittests.cpp`, for example).
