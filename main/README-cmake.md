...
`ctest` plays the role of `make check` and runs the same tests. Directories
whose Makefile.am had a `check-TESTS` rule but no `TESTS` variable were never
run by `make check`, so they aren't registered either. Tcl suites are judged by
their tcltest summary (`Total ... Failed N`), because the Tcl drivers exit 0
even when tests fail. As before, many tests expect NSCLDAQ to be installed first (the Tcl tests load packages from
...
Each package is configured for the final prefix but installed into its own
directory under `build/incorp-stage` while building. NSCLDAQ compiles and links
against that staged copy, and `cmake --install` copies it into the prefix.
Unlike `configure`, nothing is written to the prefix before the install step.

When a package is used from the source tree, the next build notices when its
sources are edited, added, deleted or renamed, then rebuilds the package and
re-stages it from scratch, so removed files don't linger. A build with no such
change leaves the package alone.

`DESTDIR` installs stage every file, but the Tcl package indexes
(`pkgIndex.tcl`) are generated by loading the libraries, which only works when
the libraries' final prefix exists. Install directly into the prefix, as CI
does, when the Tcl packages are needed. (The Autotools build did not support
`DESTDIR` for these either.)
...
(AX_CXX_COMPILE_STDCXX_11 added no flag on modern compilers), and Tcl/Tk flags
come from `tclConfig.sh`/`tkConfig.sh`, as `m4/tcl.m4` did.

Build types: the default, RelWithDebInfo, uses exactly the Autotools flags
(`-g -O2`). Release (`-O3`) and MinSizeRel (`-Os`) also leave out CMake's usual
`-DNDEBUG`, because some code relies on `assert()` side effects (a socket read
inside an `assert()` in `daq/eventbuilder/transmittests.cpp`, for example). As
with re-running `configure`, these defaults are re-applied on every CMake run,
including in existing build directories. Flags you set explicitly
(`-DCMAKE_CXX_FLAGS_RELEASE=...`) are kept.
