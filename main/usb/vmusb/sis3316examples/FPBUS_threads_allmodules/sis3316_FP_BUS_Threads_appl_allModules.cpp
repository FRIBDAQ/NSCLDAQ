/***************************************************************************/
/*                                                                         */
/*  Filename: sis3316_FP_BUS_appl_allModules_Threads.cpp                   */
/*                                                                         */
...
/*      	  Readout will be performed in parallel with threads           */
/*                                                                         */
/*  Autor:                TH                                               */
/*  date:                 19.09.2019                                       */
...
//#define THREAD_MAX_NUMBER_ADC_MODULE_BUFFER_SIZE_IN_MBYTE			0x08000000       /* 128 MByte */
#define THREAD_MAX_NUMBER_ADC_MODULE_BUFFER_SIZE_IN_MBYTE			0x10000000       /* 256 MByte */
#define THREAD_MAX_NUMBER_CHANNEL_BUFFER_SIZE_IN_MBYTE				(THREAD_MAX_NUMBER_ADC_MODULE_BUFFER_SIZE_IN_MBYTE/16)


#define PARALELL_READOUT_WITH_THREADS
#ifndef PARALELL_READOUT_WITH_THREADS
...
unsigned int max_dma_read_buffer_length_array   ;

typedef struct {
	TThread*		handle;
	unsigned int	id;
	unsigned int	status;
	sis3316_adc* 	sis3316_adc_thread;
	int				return_code;
	unsigned int	max_dma_read_buffer_length;
	unsigned int*	dma_read_buffer;
	unsigned int	bank1_armed_flag;
	unsigned int*	got_nof_32bit_words_array;
	unsigned int	serial_no;
	//struct timeval 	start_time;
	//struct timeval	run_time;
	//struct timeval	stop_time;
	//struct timeval	finished_time;
} sis3316_readout_thread_handle_t;

void *sis3316_readout_thread_handle(void *ptr);

//double calc_time(struct timeval begin, struct timeval end);
/*===========================================================================*/
...
/******************************************************************************************/
	...
	/******************************************************************************************/
	/*                                                                                        */
	/*     Malloc Memory for all ADC boards       	                                          */
	/*                                                                                        */
	/******************************************************************************************/
	...
	// "Sequentiell "memory read buffer
	unsigned int *dma_read_buffer;
...
#ifdef PARALELL_READOUT_WITH_THREADS

	max_req_nof_32bit_words = SIS3316_ADC_MEMORY_BANK_32BIT_SIZE ; // max_request is limited by Memory Banks_Size

	unsigned int max_dma_read_buffer_length_array   ;

	max_dma_read_buffer_length_array = THREAD_MAX_NUMBER_CHANNEL_BUFFER_SIZE_IN_MBYTE;
	unsigned int *dma_read_buffer_array[MAX_NOF_SIS3316_ADCS];
	for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
		dma_read_buffer_array[i_mod] = (unsigned int *)malloc(THREAD_MAX_NUMBER_ADC_MODULE_BUFFER_SIZE_IN_MBYTE); //
		if(dma_read_buffer_array[i_mod] == NULL){
			printf("Error allocating dma_read_buffer_array[%d] !\n", i_mod);
			program_stop_and_wait();
		}
	}

	unsigned int *dma_read_got_nof_32bit_words_array[MAX_NOF_SIS3316_ADCS];
	for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
		dma_read_got_nof_32bit_words_array[i_mod] = (unsigned int *)malloc(16*4); //  16 x 32-words
		if(dma_read_got_nof_32bit_words_array[i_mod] == NULL){
			printf("Error allocating dma_read_got_nof_32bit_words_array[%d] !\n", i_mod);
			program_stop_and_wait();
		}
	}
...
char sis3316_readout_thread_name [32];
sis3316_readout_thread_handle_t sis3316_readout_threads[MAX_NOF_SIS3316_ADCS];


		for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
		   sprintf(sis3316_readout_thread_name, "sis3316_readout%d", i_mod);
		   printf("Generating Thread %s\n", sis3316_readout_thread_name);
		   sis3316_readout_threads[i_mod].handle = new TThread(sis3316_readout_thread_name, sis3316_readout_thread_handle, (void*) &sis3316_readout_threads[i_mod]);
		   sis3316_readout_threads[i_mod].id = i_mod;
		   sis3316_readout_threads[i_mod].status = Finished;
		   sis3316_readout_threads[i_mod].sis3316_adc_thread = sis3316_adc_array[i_mod]; // Module handle
		   sis3316_readout_threads[i_mod].dma_read_buffer = dma_read_buffer_array[i_mod]; // Memory Pointer
		   sis3316_readout_threads[i_mod].got_nof_32bit_words_array = dma_read_got_nof_32bit_words_array[i_mod]; // Memory Pointer
		   sis3316_readout_threads[i_mod].max_dma_read_buffer_length = max_dma_read_buffer_length_array; // read_buffer_length
		}
...
//#define PARALELL_READOUT_WITH_THREADS
		...
		unsigned char wait_of_threads = 0;

		unsigned int*	uint_thread_got_nof_32bit_words_ch_array_ptr;


		for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
			uint_thread_got_nof_32bit_words_ch_array_ptr = dma_read_got_nof_32bit_words_array[i_mod] ;
			for (i_ch=0; i_ch<16; i_ch++) {
				uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch] = 0 ;
			}
		}
		// Start Threads
		...
		for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
			//uint_thread_got_nof_32bit_words_ptr = &uint_thread_got_nof_32bit_words[i_mod][0] ;
			sis3316_readout_threads[i_mod].bank1_armed_flag = bank1_armed_flag;
			//sis3316_readout_threads[i_mod].got_nof_32bit_words = uint_thread_got_nof_32bit_words_ptr;
			sis3316_readout_threads[i_mod].status = Started;
			sis3316_readout_threads[i_mod].handle->Run();
		}
		printf("Readout Threats started\n");
		//Finished = 0,
		//Started,
		//Run,
#ifdef CERN_ROOT_PLOT
for (i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
	...
	gSystem->ProcessEvents();  // handle GUI events
		...
		do{
			wait_of_threads = 0;
			for(i=0; i<MAX_NOF_SIS3316_ADCS; i++)	{
				//printf(" i = %d    sis3316_readout_threads[i].status = %d   \n", i, sis3316_readout_threads[i].status);
				wait_of_threads |= sis3316_readout_threads[i].status; // wait for finished
			}
			usleep(10);
			//printf("\n\n");
			//usleep(1000000);
		} while(wait_of_threads);

		printf("Readout Threads finished  loop_counter = %d\n", loop_counter);
...
//#define THREAD_TEST_DEBUG
		...
		for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
			printf("sis3316_readout_threads%d:   return_code = 0X%08x    \n",i_mod, sis3316_readout_threads[i_mod].return_code);
			uint_thread_got_nof_32bit_words_ch_array_ptr = dma_read_got_nof_32bit_words_array[i_mod] ;
			for(i_ch=0; i_ch<16; i_ch++) {
				printf("i_ch %d  \tgot_nof_32bit_words = 0X%08x    \n",i_ch, uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch]);
			}
			if(sis3316_readout_threads[i_mod].return_code != 0) {
				usleep(5000000);
			}
			printf("\n");
			//printf("  Thread h%d: serial_number = %d\n", i, sis3316_readout_threads[i].serial_no&0xffff);
		}
#endif


		unsigned int dma_thread_buffer_ch_offset ;

		unsigned int *dma_thread_buffer_ptr;
		unsigned int *dma_ch_buffer_ptr;
		dma_thread_buffer_ch_offset = THREAD_MAX_NUMBER_CHANNEL_BUFFER_SIZE_IN_MBYTE/4 ;


		for (i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
			uint_thread_got_nof_32bit_words_ch_array_ptr = dma_read_got_nof_32bit_words_array[i_mod] ;
			dma_thread_buffer_ptr = dma_read_buffer_array[i_mod] ;
			if(sis3316_readout_threads[i_mod].return_code != 0) {
				printf("sis3316_readout_threads%d:   return_code = 0X%08x    \n",i_mod, sis3316_readout_threads[i_mod].return_code);
				gl_stopReq = TRUE;
				usleep(100000);
				...
				for (i_ch=0; i_ch<16; i_ch++) {
					dma_ch_buffer_ptr     = &dma_thread_buffer_ptr[i_ch * dma_thread_buffer_ch_offset] ;
					//return_code = sis3316_adc_array[i_mod]->read_DMA_Channel_PreviousBankDataBuffer(bank1_armed_flag /*bank2_read_flag*/, i_ch /* 0 to 15 */, max_req_nof_32bit_words, &got_nof_32bit_words, dma_read_buffer ) ; // read maximun (all) events
					//printf("read_DMA_Channel_PreviousBankDataBuffer: i_ch %d  got_nof_32bit_words = 0x%08x  \n",i_ch,  got_nof_32bit_words);
					//if (return_code != 0) {
					//	printf("read_DMA_Channel_PreviousBankDataBuffer: return_code = 0x%08x\n", return_code);
					...
					//}
					if(uint_save_raw_sample_first_event_only_mode == 0) {
						ch_event_counter = (uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch]  / event_length) ;
					}
					else {
						if(uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch] < event_length) {
							ch_event_counter = 0 ;
						}
						else {
							ch_event_counter = 1 + ((uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch] - event_length)  / header_length) ;
						}
					}
					//printf("got_nof_32bit_words %d   event_length = %d   header_length = %d   ch_event_counter = %d \n", got_nof_32bit_words, event_length, header_length,ch_event_counter);
					uint_channel_event_counter[i_mod][i_ch]  = uint_channel_event_counter[i_mod][i_ch] + ch_event_counter;
					...
					if (ch_event_counter > 0) {
						...
						if (uint_WriteData_to_File_OpenFlag == 1) {   ; //
							uint_WritenData_to_File_32bit_words += SIS3316_WriteBankChannelHeaderToDataFile (file_WriteData_to_File_Pointer, file_header_indentifier, bank_buffer_counter, i_ch, ch_event_counter , event_length_for_file_write_header, maw_test_buffer_length, file_header_reserved) ;
							uint_WritenData_to_File_32bit_words += SIS3316_WriteBankChannelEventBufferToDataFile (file_WriteData_to_File_Pointer, dma_ch_buffer_ptr, uint_thread_got_nof_32bit_words_ch_array_ptr[i_ch])  ;
						}
						// draw graph
						#ifdef CERN_ROOT_PLOT
						//for (i_event=0; i_event<ch_event_counter; i_event++) {
						//if (i_event==0) { // plot ony 1. event
								i_event=0 ; // plot ony 1. event
								graph_raw[i_mod]->sis3316_draw_chN (uint_save_raw_sample_length, &dma_ch_buffer_ptr[i_event*(event_length) + header_length], (i_mod*16) + i_ch); //
								gSystem->ProcessEvents();  // handle GUI events
							//}
						//}
					}
...
#ifdef PARALELL_READOUT_WITH_THREADS
		for(i_mod=0; i_mod<MAX_NOF_SIS3316_ADCS; i_mod++) {
			free(dma_read_buffer_array[i_mod])  ;
			free(dma_read_got_nof_32bit_words_array[i_mod])  ;
		}
#endif
...
double calc_time(struct timeval begin, struct timeval end)
{
	...
	tbegin = 	(double)begin.tv_sec + (double)begin.tv_usec;
	tend = 		(double)end.tv_sec + (double)end.tv_usec;
...
void *sis3316_readout_thread_handle(void *ptr)
{
	sis3316_readout_thread_handle_t* pth = (sis3316_readout_thread_handle_t*)ptr;
	int return_code ;
	unsigned int data;
	unsigned int max_req_nof_32bit_words_per_channel;
	unsigned int i_ch ;
	...
	for (i_ch=0;i_ch<16;i_ch++) {
		pth->got_nof_32bit_words_array[i_ch] = 0 ;
	}
	max_req_nof_32bit_words_per_channel = THREAD_MAX_NUMBER_CHANNEL_BUFFER_SIZE_IN_MBYTE/4 ;

	//printf("Thread h%d :   pth->bank1_armed_flag = %d  max_req_nof_32bit_words_per_channel = 0X%08x\n", pth->id, pth->bank1_armed_flag, max_req_nof_32bit_words_per_channel);

	for (i_ch=0;i_ch<16;i_ch++) {
		return_code = pth->sis3316_adc_thread->read_DMA_Channel_PreviousBankDataBuffer(pth->bank1_armed_flag /*bank2_read_flag*/, i_ch /* 0 to 15 */, max_req_nof_32bit_words_per_channel, &pth->got_nof_32bit_words_array[i_ch], &pth->dma_read_buffer[i_ch * max_req_nof_32bit_words_per_channel]) ; // read maximun (all) events
		//printf("Thread h%d read_DMA_Channel_PreviousBankDataBuffer:   return_code = 0X%08x    got_no_of_words = 0X%08x  \n", pth->id, return_code,  pth->got_nof_32bit_words_array[i_ch]);
		if (return_code != 0) {
			pth->got_nof_32bit_words_array[i_ch] = 0 ;
			break;
		}
	...
	//printf("Thread h%d :   return_code =  (0X%08x) \n", pth->id, return_code);


	pth->handle->Lock();
	pth->return_code = return_code;
	pth->status = Finished;
	pth->handle->UnLock();
	//printf("Thread h%d finished!\n", pth->id);
