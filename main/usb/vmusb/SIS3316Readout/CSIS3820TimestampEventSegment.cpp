/**
 * @file CSIS3820TimestampEventSegment.cpp
 * @brief Implementation for using the CSIS3820 scaler as a timstamper.
...
// Register definitions and bits in those registers:
...
static std::map<std::string, uint32_t> outputModeValues = {
  ...
  ///////////////////////////////  Method implementations (public) ////////////////////////////////

/**
 * constructor
 *   @param name - name of the object.
 */
CSIS3820TimestampEventSegment::CSIS3820TimestampEventSegment(const char* name) :
 ...
 * destructor
 */
CSIS3820TimestampEventSegment::~CSIS3820TimestampEventSegment() {
...
CSIS3820TimestampEventSegment::getConfiguration() {
 ...
 */
std::string
CSIS3820TimestampEventSegment::getName() const {
    return m_name;
}

/**
 * initialize
 *    Called after configuration to initialize the module.
 * At this point the Globals::pUSBController is also know to be valid.
 */
void
CSIS3820TimestampEventSegment::initialize() {
    CVMUSB& controller = *Globals::pUSBController;

    // This has been shamelessly lifted from the VMUSBReadout stuff but hacked about so that
    // only timestamp mode is used:
...
/**
 * read
 *    Read channel 0 and its extension as well as the same for channel 16.  So that's a total of 
 *    6 words (each timestamp is 3*16bits).
 * 
 * @param pBuffer - where to put the data.
 * @param maxwords - maximum number of 16 bit words the buffer can take at this time.
 * @return size_t number of words we read (6).
 */
size_t
CSIS3820TimestampEventSegment::read(void* pBuffer, size_t maxwords) {
    if (maxwords < 6) {
    ...
    uint32_t highbits;           // high bits of both.

    Globals::pUSBController->vmeRead32(
        base  + ShadowCounters+0*sizeof(uint32_t), CVMUSBReadoutList::a32UserData, &low0
    );
    Globals::pUSBController->vmeRead32(
        base  + ShadowCounters+16*sizeof(uint32_t), CVMUSBReadoutList::a32UserData, &low16
    );
    Globals::pUSBController->vmeRead32(
        base  + HighBits, CVMUSBReadoutList::a32UserData, &highbits
    );
...
/**
 *  setupConfiguration  - Define the configuration parameters and their validations.
 */
void
