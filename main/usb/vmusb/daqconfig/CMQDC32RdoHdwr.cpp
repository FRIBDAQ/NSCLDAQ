...
void CMQDC32RdoHdwr::configureTimeBaseSource(CVMUSBReadoutList& list) {
  ...
  if ((m_pConfig->getBoolParameter("-customecl") && m_pConfig->getIntegerParameter("-eclgate1osc") == 1)
      || (m_pConfig->getBoolParameter("-customnim") && m_pConfig->getIntegerParameter("-nimgate1osc") == 1)) {
    id = id&0x1 + 0x2;
  } // The conditions must be separate as the above ignores the below
  else if (m_pConfig->getBoolParameter("-ecltiming") || m_pConfig->getBoolParameter("-nimtiming")) {
    id = id&0x1 + 0x2;
  }
