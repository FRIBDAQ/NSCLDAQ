...
class CSRA(QWidget):
    ...
    def __init__(self, module=None, nchannels=16, *args, **kwargs):
        ...
        self.param_labels = {
            0: {
                "label": "FTrS",
                "tooltip": "Fast trigger from system FPGA (=1) or local (=0)",
            },
            1: {
                "label": "ETrS",
                "tooltip": "Validation trigger from LVDS (=1) or sys. FPGA (=0)",
            },
            2: {"label": "Good", "tooltip": "Enable (=1) or disable (=0) channel"},
            3: {
                "label": "CTrS",
                "tooltip": "Channel validation from LVDS (=1) or sys. FPGA (=0)",
            },
            4: {
                "label": "Sync",
                "tooltip": "Disable acquisition in this channel if any channel DPM on this module is full (=1) or only if this channel DPM is full (=0)",
            },
            5: {
                "label": "SPol",
                "tooltip": "Signal polarity (set on AnalogSignal tab)",
            },
            ...
            11: {
                "label": "MTrV",
                "tooltip": "Enable (=1) or disable (=0) module validation trigger",
            },
            12: {"label": "ES", "tooltip": "Enable (=1) or disable (=0) energy sums"},
            13: {
                "label": "CTrV",
                "tooltip": "Enable (=1) or disable (=0) channel validation trigger",
            },
            14: {"label": "HiG", "tooltip": "Gain (set on AnalogSignal tab)"},
            15: {
                "label": "PC1",
                "tooltip": "Pileup rejection, see manual Sec. 3.3.7, Table 3-2",
            },
            16: {
                "label": "PC2",
                "tooltip": "Inverse pileup rejection, see manual Sec. 3.3.7, Table 3-2",
            },
            17: {
                "label": "ECut",
                "tooltip": "Enable (=1) or disable (=0) energy cut trace rejection",
            },
            18: {
                "label": "GTrS",
                "tooltip": "Channel fast trigger selection, depends on bit 0 see manual Sec. 3.3.7 Table 3-2",
            },
            19: {
                "label": "CVtS",
                "tooltip": "Channel veto selection from system FPGA (=1) or LVDS (=0)",
            },
            20: {
                "label": "MVtS",
                "tooltip": "Module veto selection from system FPGA (=1) or LVDS (=0)",
            },
            21: {
                "label": "EnTS",
                "tooltip": "Enable (=1) or disable (=0) recording of timestamp in event header",
            },
        }
        ...
        # by bit+2 where bit is the CSRA bit value (key of the parameter
        # label dict):
        ...
        self.param_grid.addWidget(QLabel("Value"), 0, 1)
        for bit, pdict in self.param_labels.items():
            w = QLabel(pdict["label"])
            w.setFixedHeight(20)
            w.setToolTip(pdict["tooltip"])
            self.param_grid.addWidget(w, 0, bit + 2, Qt.AlignCenter)
        ...
        for bit, pdict in self.param_labels.items():
            ...
            if pdict["label"] in disabled:
                cb.setEnabled(False)
            cb.stateChanged.connect(lambda state, bit=bit: self._select_all(bit, state))
            self.param_grid.addWidget(cb, row + 1, bit + 2, Qt.AlignCenter)
    ...
    def update_dsp(self, mgr, mod):
        ...
        for i in range(self.nchannels):
            ...
            for bit in self.param_labels:
                csra[bit] = (
                    self.param_grid.itemAtPosition(i + 1, bit + 2).widget().isChecked()
                )
    ...
    def display_dsp(self, mgr, mod):
        ...
        for i in range(self.nchannels):
            ...
            self.param_grid.itemAtPosition(i + 1, 1).widget().setText(str(ba2int(csra)))
            for bit, val in zip(self.param_labels, csra):
                self.param_grid.itemAtPosition(i + 1, bit + 2).widget().setChecked(val)
    ...
    def _select_all(self, bit, state):
        ...
        """
        for i in range(self.nchannels):
            self.param_grid.itemAtPosition(i + 1, bit + 2).widget().setChecked(state)
