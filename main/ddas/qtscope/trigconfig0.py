...
class TrigConfig0(QWidget):
    ...
    def __init__(self, nmodules=None, nchannels=16, *args, **kwargs):
        ...
        self.rb_dict = {0: {"name": "Default (self-trigger)"}, 1: {"name": "Custom"}}
        ...
        self.param_labels = {
            0: {
                "label": "IFTr",
                "bit_low": xia.TC0_INT_FAST_TRIG_OFFSET,
                "bit_high": xia.TC0_INT_FAST_TRIG_END,
                "tooltip": "Internal fast trigger signal from channel fast trigger",
            },
            1: {
                "label": "EFTr",
                "options": [
                ...
                "bit_low": xia.TC0_EXT_FAST_TRIG_OFFSET,
                "bit_high": xia.TC0_EXT_FAST_TRIG_END,
                "tooltip": "External fast trigger source",
            },
            2: {
                "label": "IVTr",
                "bit_low": xia.TC0_INT_VAL_TRIG_OFFSET,
                "bit_high": xia.TC0_INT_VAL_TRIG_END,
                "tooltip": "Internal validation trigger signal from channel fast trigger",
            },
            3: {
                "label": "TsGr",
                "options": ["TTL0", "TTL1"],
                "bit_low": xia.TC0_TEST_GROUP_OFFSET,
                "bit_high": xia.TC0_TEST_GROUP_END,
                "tooltip": "Group test signal sent to digital output for test/debug",
            },
            4: {
                "label": "TsEn",
                "options": ["Disabled", "Enabled"],
                "bit_low": xia.TC0_ENB_TEST_OFFSET,
                "bit_high": xia.TC0_ENB_TEST_END,
                "tooltip": "Send test signals to digital output for test/debug",
            },
            5: {
                "label": "TsCh",
                "bit_low": xia.TC0_CH_TEST_OFFSET,
                "bit_high": xia.TC0_CH_TEST_END,
                "tooltip": "Channel test signal to digital output for test/debug",
            },
            6: {
                "label": "TsDO",
                "options": [
                ...
                "bit_low": xia.TC0_6TH_TEST_OFFSET,
                "bit_high": xia.TC0_6TH_TEST_END,
                "tooltip": "6th output of digital output for test/debug",
            },
            7: {
                "label": "MFTr",
                "options": [
                ...
                "bit_low": xia.TC0_MOD_FAST_TRIG_OFFSET,
                "bit_high": xia.TC0_MOD_FAST_TRIG_END,
                "tooltip": "Module fast trigger source",
            },
            8: {
                "label": "MVTr",
                "options": [
                ...
                "bit_low": xia.TC0_MOD_VAL_TRIG_OFFSET,
                "bit_high": xia.TC0_MOD_VAL_TRIG_END,
                "tooltip": "Module validation trigger source",
            },
            9: {
                "label": "EvTr",
                "options": [
                ...
                "bit_low": xia.TC0_EXT_VAL_TRIG_OFFSET,
                "bit_high": xia.TC0_EXT_VAL_TRIG_END,
                "tooltip": "External validation trigger source",
            },
        }
    ...
    def update_dsp(self, mgr):
        ...
        for i in range(self.nmodules):
            ...
            for j, pdict in self.param_labels.items():
                ...
                tc[pdict["bit_low"] : pdict["bit_high"]] = int2ba(
                    int(
                        self.param_grid.itemAtPosition(i + 1, j + 1)
                        .widget()
                        .currentIndex()
                    ),
                    pdict["bit_high"] - pdict["bit_low"],
                    "little",
                )
    ...
    def display_dsp(self, mgr, set_state=False):
        ...
        for i in range(self.nmodules):
            ...
            for j, pdict in self.param_labels.items():
                self.param_grid.itemAtPosition(i + 1, j + 1).widget().setCurrentIndex(
                    ba2int(tc[pdict["bit_low"] : pdict["bit_high"]])
                )
        ...
        if self.rbgroup.checkedId() == 0:
            for i in range(self.nmodules):
                for j, _ in enumerate(self.param_labels):
                    self.param_grid.itemAtPosition(
                        i + 1, j + 1
        ...
        if self.rbgroup.checkedButton().text() == "Custom":
            for i in range(self.nmodules):
                for j in self.param_labels:
                    self.param_grid.itemAtPosition(i + 1, j + 1).widget().setEnabled(
                        True
                    )
        else:
            for i in range(self.nmodules):
