...
class MultCoincidence(QWidget):
    ...
    def __init__(self, *args, nchannels=16, **kwargs):
        ...
        **kwargs : dict
        ...
        self.coinc_width.setValidator(
            QDoubleValidator(0, 999999, 3, notation=QDoubleValidator.StandardNotation)
        )
        ...
        self.rbgroup.buttonClicked.connect(self._configure_multiplicity_threshold)
    ...
    def configure(self, mgr, mod):
        ...
        for i in range(self.nchannels):
            ...
            mask = int2ba(
                int(mgr.get_chan_par(mod, i, "MultiplicityMaskH")), 32, "little"
            )
        ...
        try:
            if not all(enb == enb_list[0] for enb in enb_list):
                raise ValueError(f"Custom CSRA settings detected on Mod. {mod}")
            if enb_list[0]:
    ...
    def display_dsp(self, mgr, mod):
        ...
        val = np.format_float_positional(
            mgr.get_chan_par(mod, 0, "ChanTrigStretch"), precision=3, unique=False
        )
        ...
        mask = int2ba(int(mgr.get_chan_par(mod, 0, "MultiplicityMaskH")), 32, "little")
        mult = mask[xia.MULT_OFFSET : xia.MULT_END]
    ...
    def print_masks(self, mgr, mod):
        ...
        for i in range(self.nchannels):
            low_mask = int2ba(
                int(mgr.get_chan_par(mod, i, "MultiplicityMaskL")), 32, "little"
            )
            high_mask = int2ba(
                int(mgr.get_chan_par(mod, i, "MultiplicityMaskH")), 32, "little"
            )
    ...
    def _get_channel_group(self, mgr, mod):
        ...
        print(
            f"{self.__class__.__name__}.{inspect.currentframe().f_code.co_name}: Encountered unknown channel multiplicity state!"
        )
        for idx, mode in self.mode_dict.items():
            if mode["name"] == "Unknown":
                return idx
    ...
    def _set_channel_group(self, mgr, mod):
        ...
        try:
            ...
            for i in range(self.nchannels):
                ...
                # Write the mask:
                mgr.set_chan_par(mod, i, "MultiplicityMaskL", float(ba2int(mask)))
