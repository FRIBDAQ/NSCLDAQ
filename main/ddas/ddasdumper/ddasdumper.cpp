 ...
 */
static FormatSelector::SupportedVersions mapVersion(enum_nscldaq_format fmtIn) {
  switch (fmtIn) {
 ...
 */
static void dumpItem(::ufmt::CRingItem *pItem,
                     ::ufmt::RingItemFactoryBase &factory,
  ...
  case PAUSE_RUN:
  case RESUME_RUN: {
    std::unique_ptr<CRingStateChangeItem> p(
        factory.makeStateChangeItem(*pItem));
  ...
  case PHYSICS_EVENT_COUNT: {
    std::unique_ptr<CRingPhysicsEventCountItem> p(
        factory.makePhysicsEventCountItem(*pItem));
    std::cout << p->toString() << std::endl;
  } break;
  case EVB_FRAGMENT:
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::unique_ptr<DataSource> pSource(makeDataSource(&factory, dataSource));
