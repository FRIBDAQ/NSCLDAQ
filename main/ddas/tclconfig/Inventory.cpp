...
CInventory::CInventory(Tcl_Interp *pInterp, DAQ::DDAS::Configuration &config)
    : CTclCommand(pInterp, "pixie16::inventory"), m_config(config) {}
 ...
 *
 */
Tcl_Obj *CInventory::describeModule(int slot, int rev, int ser, int bits,
                                    int mhz) {
 ...
 */
void CInventory::freeObjects(Tcl_Obj *list, std::vector<Tcl_Obj *> &elements) {
