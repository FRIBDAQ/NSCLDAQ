...
CTclCommand::CTclCommand(Tcl_Interp *pInterp, const char *cmdName)
  ...
  Tcl_CreateObjCommand(m_pInterp, m_cmdName.c_str(), commandRelay, this,
                       nullptr);
}
/**
 * destructor unregisters the command
 */
CTclCommand::~CTclCommand() { Tcl_DeleteCommand(m_pInterp, m_cmdName.c_str()); }
////////////////////////////////////////////////////////////////
 ...
 */
void CTclCommand::requireExactly(std::vector<Tcl_Obj *> &objv, int nParams) {
 ...
 */
void CTclCommand::requireAtMost(std::vector<Tcl_Obj *> &objv, int max) {
  if (objv.size() > max) {
    std::stringstream s;
 ...
 *
 */
int CTclCommand::commandRelay(ClientData pObject, Tcl_Interp *pInterp, int objc,
                              Tcl_Obj *const objv[]) {
