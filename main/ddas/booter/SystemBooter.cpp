...
void DAQ::DDAS::SystemBooter::boot(Configuration &config, BootType type) {
  ...
  int retval =
      Pixie16InitSystem(NumModules, config.getSlotMap().data(), m_offlineMode);
  if (retval < 0) {
...
void DAQ::DDAS::SystemBooter::bootModuleByIndex(int modIndex,
  ...
  FirmwareConfiguration fwConfig =
      config.getModuleFirmwareConfiguration(hdwrMap[modIndex], modIndex);
  ...
  if (m_verbose) {
    if (type == FullBoot) {
      std::cout << "\nBooting Pixie-16 module #" << modIndex << std::endl;
      std::cout << "\tComFPGAConfigFile:  " << Pixie16_Com_FPGA_File
                << std::endl;
      std::cout << "\tSPFPGAConfigFile:   " << Pixie16_SP_FPGA_File
                << std::endl;
      std::cout << "\tDSPCodeFile:        " << Pixie16_DSP_Code_File
                << std::endl;
      std::cout << "\tDSPVarFile:         " << Pixie16_DSP_Var_File
                << std::endl;
      std::cout << "\tDSPParFile:         " << DSPParFile << std::endl;
      std::cout << "------------------------------------------------------";
      std::cout << "\n\n";
    } else {
  ...
  int retval = Pixie16BootModule(Pixie16_Com_FPGA_File, Pixie16_SP_FPGA_File,
                                 Pixie16_Trig_FPGA_File, Pixie16_DSP_Code_File,
                                 DSPParFile, Pixie16_DSP_Var_File, modIndex,
  ...
  // TEST OVERRIDE SLOT ID
  auto slots = config.getSlotMap();
  retval = Pixie16WriteSglModPar("SlotID", slots[modIndex], modIndex);
  if (retval != 0) {
    std::stringstream errmsg;
    errmsg << "Failed to write SlotID for module " << modIndex
           << " with Pixie16WriteSglModPar() retval = " << retval << "!";
    throw std::runtime_error(errmsg.str());
  }
}
 ...
 */
void DAQ::DDAS::SystemBooter::setVerbose(bool enable) { m_verbose = enable; }
...
void DAQ::DDAS::SystemBooter::populateHardwareMap(Configuration &config) {
  ...
  for (unsigned short k = 0; k < NumModules; k++) {
    int retval =
        Pixie16ReadModuleInfo(k, &ModRev, &ModSerNum, &ModADCBits, &ModADCMSPS);
    if (retval < 0) {
      std::stringstream errmsg;
      ...
      auto type =
          HardwareRegistry::computeHardwareType(ModRev, ModADCMSPS, ModADCBits);
      hdwrMapping[k] = type;
    }
  }
 ...
 * signatures/calls?
 */
void DAQ::DDAS::SystemBooter::logModuleInfo(int modIndex, unsigned short ModRev,
