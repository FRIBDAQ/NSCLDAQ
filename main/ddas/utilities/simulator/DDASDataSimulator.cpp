...
const uint16_t UINT16_T_MASK = 0xFFFF;     //!< Mask for uint16_t values.
 ...
 */
static FormatSelector::SupportedVersions mapVersion(int version) {
  switch (version) {
  case 12:
 ...
 */
void DAQ::DDAS::DDASDataSimulator::beginRun(int sourceID) {
 ...
 */
void DAQ::DDAS::DDASDataSimulator::endRun(int sourceID) {
  const auto now = std::chrono::system_clock::now();
  ...
  if (close(m_fd) == -1) {
    std::string msg("DDASDataSimulator::endRun() failed to close output file ");
    msg += m_fname;
    throw CErrnoException(msg);
  }
}
 ...
 */
void DAQ::DDAS::DDASDataSimulator::putHit(const DDASHit &hit, int sourceID,
                                          bool useExtTS, double cal) {
  ...
  std::unique_ptr<CPhysicsEventItem> pPhysicsItem(
      m_pFactory->makePhysicsEventItem(
          timestamp, sourceID, 0,
          bodySize)); // Already has a correct body header.
 ...
 */
void DAQ::DDAS::DDASDataSimulator::setBuffer(const DDASHit &hit) {
 ...
 */
void DAQ::DDAS::DDASDataSimulator::setWord0(const DDASHit &hit) {
  ...
  uint32_t hwRev = hit.getHardwareRevision();

  // Upper 20 bits are the same regardless of hardware revision, so set those
  // first:

  uint32_t word = 0x0;
  word |= (finishCode & (FINISH_CODE_MASK >> FINISH_CODE_SHIFT))
          << FINISH_CODE_SHIFT;
  word |= (chanLen & (CHANNEL_LENGTH_MASK >> CHANNEL_LENGTH_SHIFT))
          << CHANNEL_LENGTH_SHIFT;
  word |= (hdrLen & (HEADER_LENGTH_MASK >> HEADER_LENGTH_SHIFT))
          << HEADER_LENGTH_SHIFT;

  if (hwRev == 0 || hwRev > 17) {
    std::stringstream msg;
    msg << "Invalid hardware revision " << hwRev
        << " for Pixie list-mode format! Valid revisions are integer values"
        << " corresponding to B (11), C (12), D (13), F (15), H (17).";
    throw std::runtime_error(msg.str());
  } else if (hwRev == 17) {
    word |= (crate & (CRATE_ID_MASK_REV_H >> CRATE_ID_SHIFT_REV_H))
            << CRATE_ID_SHIFT_REV_H;
    word |= (slot & (SLOT_ID_MASK_REV_H >> SLOT_ID_SHIFT_REV_H))
            << SLOT_ID_SHIFT_REV_H;
    word |= chan & CHANNEL_ID_MASK_REV_H;
  } else {
    word |= (crate & (CRATE_ID_MASK >> CRATE_ID_SHIFT)) << CRATE_ID_SHIFT;
    word |= (slot & (SLOT_ID_MASK >> SLOT_ID_SHIFT)) << SLOT_ID_SHIFT;
    word |= chan & CHANNEL_ID_MASK;
  }
 ...
 */
void DAQ::DDAS::DDASDataSimulator::setWords1And2(const DDASHit &hit) {
 ...
 */
void DAQ::DDAS::DDASDataSimulator::setWord3(const DDASHit &hit) {
...
void DAQ::DDAS::DDASDataSimulator::setExternalTS(const DDASHit &hit) {
 ...
 * requirement.
 */
void DAQ::DDAS::DDASDataSimulator::setEnergySums(const DDASHit &hit) {
 ...
 * requirement.
 */
void DAQ::DDAS::DDASDataSimulator::setQDCSums(const DDASHit &hit) {
 ...
 */
void DAQ::DDAS::DDASDataSimulator::setTraceData(const DDASHit &hit) {
  auto trace = hit.getTrace();
  int len = trace.size();
 ...
 */
uint32_t DAQ::DDAS::DDASDataSimulator::getHeaderLength(const DDASHit &hit) {
...
uint32_t DAQ::DDAS::DDASDataSimulator::getModInfoWord(const DDASHit &hit) {
  int rev = hit.getHardwareRevision();
  int bits = hit.getADCResolution();
  int msps = hit.getModMSPS();

  uint32_t word = 0x0;
  word |= (rev & (HW_REVISION_MASK >> HW_REVISION_SHIFT)) << HW_REVISION_SHIFT;
  word |= (bits & (ADC_RESOLUTION_MASK >> ADC_RESOLUTION_SHIFT))
          << ADC_RESOLUTION_SHIFT;
  word |= msps; // No shift.
 ...
 */
uint64_t DAQ::DDAS::DDASDataSimulator::getCoarseTimestamp(const DDASHit &hit) {
  double time = hit.getTime();
...
int DAQ::DDAS::DDASDataSimulator::getClockPeriod(const DDASHit &hit) {
  int clockPeriod;
  int msps = hit.getModMSPS();
  if (msps == 100) {
    clockPeriod = 10;
...
int DAQ::DDAS::DDASDataSimulator::getSamplePeriod(const DDASHit &hit) {
  int samplePeriod;
  int msps = hit.getModMSPS();
  if (msps == 100) {
    samplePeriod = 10;
 ...
 */
uint32_t DAQ::DDAS::DDASDataSimulator::getPackedCFDResult(const DDASHit &hit,
                                                          double corr) {
  uint32_t result = 0x0;
  double rawCFD;
  ...
  if (msps == 100) {
    ...
    if (result == 32768)
      result = 32767; // Set to max allowed
    ...
    result |= static_cast<uint32_t>(std::floor(rawCFD));
    if (result == 16384)
      result = 16383; // Set to max allowed
    ...
    result |= static_cast<uint32_t>(std::floor(rawCFD));
    if (result == 8192)
      result = 8191; // Set to max allowed
