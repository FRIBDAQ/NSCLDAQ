...
public:
   ...
   * @param sourceID The source ID value (optional, default=0).
   * @param useExtTS Use an external timestamp (optional, default=false).
   * @param cal Clock calibration in nanoseconds per clock cycle. Required if
   * useExtTS is true (optional, default=0.0).
   * @throw std::runtime_error If timestamp or external timestamp are invalid
   * (<= 0 or not provided when expected).
   * @throw std::runtime_error If using an external timestamp with an invalid
   * clock calibration (<= 0.0).
   */
  void putHit(const ddasfmt::DDASHit &hit, int sourceID = 0,
   ...
   * @return The data buffer. May or may not be empty, depending on whether
   * `setBuffer()` is called first.
   */
...
private:
  /**
   * @brief Set word 0 of the fixed Pixie list-mode event header. Contains
   * identifying information for the hit and its size.
   * @param hit References the hit to process.
   * @throws std::runtime_error If the hit hardware revision is invalid.
   */
  void setWord0(const ddasfmt::DDASHit &hit);
  /**
   * @brief Set words 1 and 2 of the fixed Pixie list-mode event header. Word 1
   * Contains lower 32 bits of 48-bit timestamp, word 2 contains upper 16 bits
   * of 48-bit timestamp and CFD correction.
   * @param hit References the hit to process.
   * @throws std::runtime_error If the hit timestamp is < 0.
   */
  void setWords1And2(const ddasfmt::DDASHit &hit);
  /**
   * @brief Set word 3 of the fixed Pixie list-mode event header. Contains
   * energy and trace length, overflow.
   * @param hit References the hit to process.
   */
   ...
   */
  uint32_t getPackedCFDResult(const ddasfmt::DDASHit &hit, double corr);
};
