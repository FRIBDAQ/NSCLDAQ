...
void DDASReadoutMain::SetupReadout(CExperiment *pExperiment) {
  ...
  CMyEndCommand *pMyEnd =
      new CMyEndCommand(*pInterp, myeventsegment, pExperiment);
  ...
  CBootCommand *pBootCommand =
      new CBootCommand(*pInterp, "ddas_boot", myeventsegment);
}
...
void DDASReadoutMain::SetupScalers(CExperiment *pExperiment) {
  ...
  if (modules > MAX_MODULES_PER_CRATE) {
    cerr << "**ERROR** Attempting to setup scalers for " << modules
         << " when a max of " << MAX_MODULES_PER_CRATE << " are allowed!"
         << endl;
  }
...
void DDASReadoutMain::addCommands(CTCLInterpreter *pInterp) {
  ...
  new CDDASStatisticsCommand(*pInterp, "statistics", myeventsegment,
                             scalerModules);
}
