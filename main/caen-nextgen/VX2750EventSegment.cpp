 ...
 *   @param pConfig     - The configuration database which will have our module
 * config. Note that the caller retains ownership of the configuration.
 ...
 *                        This parameter is optional and defaults to fals
 * (ethernet).
 */
VX2750EventSegment::VX2750EventSegment(CExperiment *pExperiment,
                                       uint32_t sourceId,
                                       const char *pModuleName,
                                       VX2750TclConfig *pConfig,
                                       const char *pHostOrPid, bool fIsUsb

                                       )
    : m_pExperiment(pExperiment), m_sourceId(sourceId), m_pModule(nullptr),
      m_pConfiguration(pConfig), m_moduleName(pModuleName),
 ...
 */
void VX2750EventSegment::hwInit() {
  try {
 ...
 *          derived exceptions so those too get mapped into std::string
 * exceptions which I'm sure it does understand.
 */
void VX2750EventSegment::initialize() {

  try {
    if (!m_pModule) {
      throw std::logic_error(
          "The module object should have been created but was not yet");
    }
    auto pConfig = m_pConfiguration->getModule(m_moduleName.c_str());
    ...
    for (auto s : startSources) {
      if (s == "SWcmd") {
        std::cout << "SWcmd is enabled as a start source for " << m_moduleName
                  << " so invoking start\n";
        m_pModule->Start();
 ...
 *
 */
void VX2750EventSegment::disable() {
 ...
 */
void VX2750EventSegment::onPause() { disable(); }
/**
 * onResume
 ...
 *    +------------------------------------|
 *    | uint32_t digital probe bytes       | 0 if no data (e.g. disabled)
 *    +------------------------------------+
 *    |  Digital probe data(1)             | 1 byte per sample
 *             ...                           could be no data
 *    +------------------------------------+
 *    |  Digital probe type 2 (*)          | uint16_t
 *    +------------------------------------|
 *    | uint32_t digital probe bytes       | 0 if no data (e.g. disabled)
 *    +------------------------------------+
 ...
 *             ...                           could be no data
 *    +------------------------------------+
 *    |  Digital probe type 4 (*)          | uint16_t
 *    +------------------------------------|
 *    | digital probe bytes                | 0 if no data (e.g. disabled)
 *    +------------------------------------+
 *    |  Digital probe data(4)             | 1 byte per sample
 *             ...                           could be no data
 *    +------------------------------------+
 *
 *\endverbatim
 ...
 */
size_t VX2750EventSegment::read(void *pBuffer, size_t maxwords) {
  ...
  size_t bytesNeeded = m_moduleName.size() + 1 + 7 * sizeof(uint16_t) +
                       2 * sizeof(uint64_t); // Fixed junk:
  if (m_moduleName.size() % 2 == 0)
    bytesNeeded++; // Round to next uint16_t
  bytesNeeded +=
      6 * (sizeof(uint16_t) + sizeof(uint32_t)); // Always present probe stuff.
  ...
  while (bytesNeeded % sizeof(uint16_t) > 0)
    bytesNeeded++;
  ...
  if (bytesNeeded > (maxwords * sizeof(uint16_t))) {
    ...
    strMsg << "Reading out module " << m_moduleName << " channel "
           << m_Event.s_channel << " requires " << bytesNeeded
           << " bytes but there's only " << maxwords * sizeof(uint16_t)
           << " bytes available\n";
    strMsg << " Either increase the event buffer length or decrease the "
  ...
  *p.p16++ = m_Event.s_fail; // Issue #356: last thing is the fail bit:
  ...
  *p.p16++ = m_Event.s_digitalProbe3Type;
  if (m_Event.s_pDigitalProbe3) {
    *p.p32++ = digitalProbeLength;
