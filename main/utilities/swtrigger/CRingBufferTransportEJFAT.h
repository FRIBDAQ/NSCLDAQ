 ...
 */
#ifndef CRINGBUFFERTRANSPORTEJFAT_H
#define CRINGBUFFERTRANSPORTEJFAT_H
 ...
 *      operations.
 */
class CRingBufferTransportEJFAT : public CRingItemTransport
{
private:
    ...
    size_t m_timeoutMs; //!< Milliseconds to idle after data has been processed
    bool m_timedOut; //!< Whether or not we've timed out a read
    bool m_seenData; //!< True if some real data has been observed
    
public:
    CRingBufferTransportEJFAT(CRingBuffer& writer);           // For writes.
    CRingBufferTransportEJFAT(CRingBufferChunkAccess& reader); // for reads.
    virtual ~CRingBufferTransportEJFAT();
